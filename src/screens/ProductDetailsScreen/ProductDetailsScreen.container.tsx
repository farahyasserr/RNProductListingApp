...
function ProductDetailsScreen({navigation, route}: Props) {
    ...
    navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity
          ...
          <Heart
            fill={isWishList ? colors.primary : colors.transparent}
            size={20}
            color={isWishList ? colors.primary : colors.black}
          />
        </TouchableOpacity>
      ),
    });
  ...
  return <ProductDetailsView isLoading={isLoading} productItem={productItem} />;
}
